...
var launchMongo = function (options) {
  ...
  var initiateReplSetAndWaitForReady = function () {
    ...
    if (initiateResult && initiateResult.documents
      ...
      if (err !== "already initialized") {
        throw Error("rs.initiate error: " + initiateResult.documents[0].errmsg);
      }
    }

    // XXX timeout eventually?
    while (true) {
      var status = yieldingMethod(db.admin(), 'command', {replSetGetStatus: 1});
      ...
      // Is the intended primary currently a secondary? (It passes through that
      // phase briefly.)
  ...
  process.nextTick(inFiber(function () {
    if (options.multiple) {
